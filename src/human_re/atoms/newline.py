...
class Newline(RegexElement, Quantifiable, Nameable):
    """
    Matches a newline character
    """

    times: Union[int, Tuple[int, ...], Range, _SpecialQuantifier] = 1
    """ The number of times newline should be matched (default: 1).  Read `src.elements.multiple.Multiple` 
    ...
    def compile(self) -> Pattern:
        return Multiple(
            RegexPattern(re.compile(r"\n")), times=self.times, name=self.name
        ).compile()
